 ...
 * ----------------------구현---------------------------------
 * 1. public class로 구현
 * 2. javax.servlet.http.HttpServlet를 상속(extends) => java EE(API)
 * 3. 매개변수 없는 생성자(no-argument생성자) => 생성자는 default 생성자 (자동생성)
 * 4. doGet() 또는 doPost() 메소드를 오버라이딩 => 서블릿이 서비스할 내용을 구현한다.
 * 		doGet() - GET방식 요청을 처리. doPost() - POST방식 요청을 처리.
 * ----------------------등록---------------------------------
 * 5. 구현한 Servlet class를 톰켓에 등록
 * 		=> 1. web.xml]
 * 			2. Annotation을 이용해 등록.(서블릿 3.0 버전부터 - 톰켓 7버전부터)
 * 		
 */
public class HelloServlet extends HttpServlet{
	public void doGet(HttpServletRequest request, HttpServletResponse response)throws IOException, ServletException{
		...
		//SimpleDateFormat : java.util.Date <--> String을 Date로 바꾸거나 Date를 String으로 바꿈
		//		Date -> String (format()사용) 			String -> Date (parse()사용)
		//yyyy : 년도, MM : 월, dd : 일, HH : 시, mm : 분, ss : 초
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String current2 = sdf.format(date);
		//Date dd = sdf.parse("2011-11-20 23:22:05"); String을 Date로 변경 Exception을 던짐
